...
public actor TURNClient {
    ...
    private func createAllocateRequest(withAuth: Bool) -> Data {
        ...
        // RFC 5389 §15.4: HMAC計算前にメッセージ長をMI属性サイズ(24バイト)分含めて更新する
        if withAuth {
    ...
    private func createPermissionRequest(peerIP: String, peerPort: UInt16) -> Data {
        ...
        // RFC 5389 §15.4: MI属性サイズ(24バイト)を含めた長さでHMAC計算
        message = updateMessageLength(message, addBytes: 24)
        let hmac = computeHMACSHA1(message: message)
    ...
    /// MESSAGE-INTEGRITY計算（HMAC-SHA1、20バイトの生HMAC値を返す）
    /// RFC 5389 §15.4準拠
    ...
    private func parseErrorResponse(_ data: Data) {
        ...
        while offset < min(20 + messageLength, data.count) {
            ...
            if attrType == TURNAttributeType.nonce.rawValue {
                ...
                // ERROR-CODE属性: class(百の位) + number(残り) + reason text
                if attrLength >= 4 {
                    let errorClass = Int(data[offset + 2])
                    let errorNumber = Int(data[offset + 3])
                    lastErrorCode = errorClass * 100 + errorNumber
                    lastErrorMessage = attrLength > 4 ? String(data: data[(offset + 4)..<(offset + attrLength)], encoding: .utf8) ?? "" : ""
    ...
    private func sendAndReceive(_ conn: NWConnection, data: Data) async throws -> Data {
        ...
        // （受信ループがconn.receiveを独占しているため直接receiveできない）
    ...
    private func processIncoming(_ data: Data) {
        ...
        // ★ STUNレスポンス（CreatePermission/ChannelBind/Refresh応答）は
        //    pending continuationに転送
    ...
    private func isSTUNResponse(_ msgType: UInt16) -> Bool {
        // STUN Success/Error Response のビットパターン:
        // Success: 0x01xx (bit 8 = 1)
        // Error:   0x01xx with bit 4 = 1 → 0x011x
        let stunResponses: [UInt16] = [
            TURNMessageType.createPermissionResponse.rawValue,  // 0x0108
            TURNMessageType.channelBindResponse.rawValue,       // 0x0109
            TURNMessageType.refreshResponse.rawValue,           // 0x0104
            TURNMessageType.allocateResponse.rawValue,          // 0x0103
            TURNMessageType.allocateErrorResponse.rawValue,     // 0x0113
